...
const objectPrefix = "publicippool-"
...
func (b *FunctionBuilder) Build() (result controllers.ReconcilerFunction[*privatev1.PublicIPPool], err error) {
	if b.logger == nil {
...
func (t *task) validateTenant() error {
	if !t.publicIPPool.HasMetadata() || len(t.publicIPPool.GetMetadata().GetTenants()) != 1 {
		return errors.New("public IP pool must have exactly one tenant assigned")
	}
	return nil
}
...
func (t *task) getKubeObject(ctx context.Context) (result *unstructured.Unstructured, err error) {
	...
	if count > 1 {
		err = fmt.Errorf(
			"expected at most one public IP pool with identifier '%s' but found %d",
			t.publicIPPool.GetId(), count,
		)
		return
	}
	if count > 0 {
